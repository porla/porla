...
namespace
{
    std::optional<std::int64_t> ToUnixTime(lt::time_point tp)
    {
        ...
        const auto age  = lt::clock_type::now() - tp;
        const auto wall = std::chrono::system_clock::now()
                        - std::chrono::duration_cast<std::chrono::system_clock::duration>(age);

        return std::chrono::duration_cast<std::chrono::seconds>(wall.time_since_epoch()).count();
    }

    std::optional<std::int64_t> NonZeroTime(std::time_t t)
    {
...
void LtTorrentStatus::Register(sol::state& lua)
{
    lua.new_usertype<lt::torrent_status>(
        ...
        "added_time",             sol::property([](const lt::torrent_status& ts) { return static_cast<std::int64_t>(ts.added_time); }),
        ...
        "completed_time",         sol::property([](const lt::torrent_status& ts) { return NonZeroTime(ts.completed_time); }),
        ...
        "last_seen_complete",     sol::property([](const lt::torrent_status& ts) { return NonZeroTime(ts.last_seen_complete); }),
        "last_upload",            sol::property([](const lt::torrent_status& ts) { return ToUnixTime(ts.last_upload); }),
