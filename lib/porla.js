...
class Porla {
     ...
     * @param {Object} [options.dht] DHT options.
     * @param {Boolean} [options.dht.enabled=true] Enable (or disable) DHT.
     * @param {Array<Array>} [options.dht.bootstrapNodes] Custom bootstrap nodes in the format [ <host>, <port> ]
     * @param {Array<Array>} [options.listenInterfaces] The listen interfaces to use in the format [ <ip or device>, <port> ]
     ...
     * @param {Boolean} [options.requireEncryption] Require encryption for peer connections.
    ...
    async _load() {
        ...
        if (this._options.dht && Array.isArray(this._options.dht.bootstrapNodes)) {
            const nodes = this._options.dht.bootstrapNodes
                .map(val => `${val[0]}:${val[1]}`)
                .join(',');
        ...
        if (this._options.dht && typeof this._options.dht.enabled === 'boolean') {
            sessionSettings.enable_dht = this._options.dht.enabled;
        }

        // Listen interfaces
        if (Array.isArray(this._options.listenInterfaces)) {
            const ifaces = this._options.listenInterfaces
                .map(val => `${val[0]}:${val[1]}`)
                .join(',');
        ...
        await this.emit('session.initSettings', { settings: sessionSettings });

        this._session = new lt.session(sessionSettings);
