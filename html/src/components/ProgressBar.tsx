...
export const ProgressBar = ({ progress, ...props }: ProgressBarProps) => {
  const relativeFillPercentage = 10000 * (progress ** -1);
  ...
  return (
    <div className="flex items-center">
      ...
      <div className="w-12 text-right">{progress < 10 ? progress.toFixed(2) : (progress < 100 ? progress.toFixed(1) : progress.toFixed(0))}%</div>
    </div>
  );
};
